...
public sealed class EverywhereMcpHttpHost : IHostedService, IAsyncDisposable
{
    ...
    private WebApplication BuildApp(int port)
    {
        ...
        // credential resolver + writer store from the parent container so
        // the inner SDK ctor injection resolves them all.
        // SPEC everywhere-connector.md §3.1.
        ...
        var credentialStore = _parentServices.GetService<Connector.JsonCredentialStore>();
        ...
        var runLog = _parentServices.GetService<Connector.RunLogStore>();
        if (runLog is not null) builder.Services.AddSingleton(runLog);
        var tokens = _parentServices.GetService<Connector.RuntimeTokenStore>();
